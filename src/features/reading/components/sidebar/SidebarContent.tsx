            ...
            if (activeBand) {
                try {
                    if (selectedTopic) {
                        ...
                        const fetchedExams = await readingService.getExamsByTopicId(selectedTopic.id);
                        ...
                        for (const exam of fetchedExams) {
                            if (exam.pdfDocuments && exam.pdfDocuments.length > 0) {
                                const readingInfo = await fetchReadingInfo(exam.pdfDocuments[0].id);
                                if (readingInfo) {
                        ...
                        const fetchedTopics = await readingService.getTopicsByProficiencyId(activeBand.id);
                        ...
                        for (const topic of fetchedTopics) {
                            const examsForTopic = await readingService.getExamsByTopicId(topic.id);
                    ...
                    setError("Không thể tải nội dung. Vui lòng kiểm tra lại API hoặc kết nối mạng.");
                } finally {
                    setLoading(false);
                }
            }
                ...
                {loading ? (
                    ...
                    selectedTopic ? (
                        ...
                        examsToDisplay.length > 0 ? (
                                ...
                                {examsToDisplay.map((exam) => (
                                    ...
                                    >
                                        <div className={styles.examCardHeader}>
                                            <span className={styles.examIcon}><EXAMS /></span>
                                            <h4>{exam.name}</h4>
                                        </div>
                            ...
                            <div className="w-full text-center py-10 text-gray-500">Chưa có đề thi nào cho topic đã chọn.</div>
                        )
                    ) : (
                                ...
                                {topics.map(topic => (
                                    <div key={topic.id} className="mb-6">
                                        ...
                                        {examsByTopic[topic.id] && examsByTopic[topic.id].length > 0 ? (
                                            ...
                                            <div className="text-gray-500">Chưa có đề thi nào cho topic này.</div>
                                        )}
                                    </div>
                                ))}
                            </div>
                        ) : (
                            <div className="w-full text-center py-10 text-gray-500">Chọn một topic từ menu bên trái để xem danh sách đề thi.</div>
                        )
                    )
                )}
            </div>
