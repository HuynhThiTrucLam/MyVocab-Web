...
const BandSidebar = ({ bands, activeBand, onSelect, onSelectTopic }: BandSidebarProps) => {
    const [expandedBands, setExpandedBands] = useState<{ [key: string]: boolean }>({});
    const [topics, setTopics] = useState<{ [bandId: string]: Topic[] }>({});
    const [loadingTopics, setLoadingTopics] = useState<{ [bandId: string]: boolean }>({});
    const [errorTopics, setErrorTopics] = useState<{ [bandId: string]: string | null }>({});
    ...
    const handleBandClick = async (band: Band) => {
        ...
        if (!topics[band.id] && !loadingTopics[band.id]) {
            ...
            try {
                ...
                setErrorTopics(prevState => ({ ...prevState, [band.id]: "Không thể tải topic." }));
            } finally {
            ...
            {bands.map((band) => (
                <div key={band.id}>
                    <button
                        ...
                        className={`${styles.bandButton} ${activeBand && activeBand.id === band.id
                    ...
                    {expandedBands[band.id] && topics[band.id] && (topics[band.id].length > 0 ? (
                        ...
                        <div className="ml-8 px-4 text-sm text-gray-500">Chưa có topic nào.</div>
                    ))}
                </div>
            ))}
        </aside>
