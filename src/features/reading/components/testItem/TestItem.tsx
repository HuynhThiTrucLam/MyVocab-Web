...
const TestItem = ({ exam, mainColor, secondaryColor }: TestItemProps) => {
        ...
        const fetchData = async () => {
            ...
            try {
                ...
                if (!pdfResponse.ok) {
                    ...
                    throw new Error(`Failed to fetch PDFs: ${pdfResponse.status} - ${errorText || pdfResponse.statusText}`);
                }
                ...
                if (!testInfoResponse.ok) {
                    ...
                    throw new Error(`Failed to fetch test info: ${testInfoResponse.status} - ${errorText || testInfoResponse.statusText}`);
                }
    ...
    const handleStartExam = () => {
        ...
        if (!pdfDocuments || pdfDocuments.length === 0) {
            ...
            alert('Không tìm thấy tài liệu PDF cho đề thi này. Vui lòng kiểm tra Exam ID: ' + exam.id);
            return;
        }
    ...
    return (
        <div className={styles.testCard}>
            ...
            <div className={styles.testCardQuestion}>
                ...
                <p>{loading ? 'Đang tải...' : testInfo ? testInfo.totalQuestions : '0'} câu hỏi</p>
            </div>
