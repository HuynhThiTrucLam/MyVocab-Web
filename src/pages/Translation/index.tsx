...
export default function Home() {
		...
		if (isSelected) {
			setSelectedItems(
				translationHistory.map(
					(item: TranslationHistoryItem) => item.id
				)
			);
		} else {
			setSelectedItems([]);
	...
	const handleTranslate = async () => {
		...
		try {
			...
			const response: any = await api.post(`/fastapi/${api_version}/translate`, {
			...
			const newHistoryItem: TranslationHistoryItem = {
				...
				targetLanguage: listSupportLanguage.find(
					(language: LanguageSupport) =>
			...
			const updatedHistory = [
				newHistoryItem,
				...translationHistory,
			].slice(0, 100); // Keep only the latest 100 items
		...
		<>
			...
			<div className="mb-10">
				<div className="grid grid-cols-1 md:grid-cols-2 gap-4">
					<div>
						<Combobox
							options={listSupportLanguage.map(
								(language: LanguageSupport) => ({
